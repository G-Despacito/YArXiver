    ...
    <v-card>
        <v-card-item>
            <v-card-title class="font-weight-semibold text-2xl">
                Paper Recommended for You according to Your Reading History
            </v-card-title>
        </v-card-item>
        <v-card-text>
...
export default {
  ...
  methods: {
    ...
    getAllRecommendPapers() {
      console.log("start getAllRecommendPapers");
      const path = "get-all-recommend-papers";
      axios
        .get(path)
        .then((res) => {
